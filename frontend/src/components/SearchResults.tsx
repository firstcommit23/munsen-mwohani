...
export default function SearchResults({
    ...
    // 지점 선택 시 results는 이미 서버에서 필터된 상태 — 카테고리 재집계
    const topCategories = useMemo(() => {
        if (!selectedStore) return Object.entries(categorySummary).slice(0, 10);
    ...
    }, [selectedStore, results, categorySummary]);

    const filtered = useMemo(() => {
        if (!deferredCategory) return results;
        return results.filter((r) => (r.category ?? '기타') === deferredCategory);
    }, [results, deferredCategory]);
    ...
    if (results.length === 0 && !chipLoading) {
        return (
            <div className="flex flex-col items-center justify-center py-20 gap-3 text-slate-400">
